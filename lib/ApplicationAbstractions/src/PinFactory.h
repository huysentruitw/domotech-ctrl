...
public:
    ...
    template<typename TState> static const std::shared_ptr<Pin> CreateInputPin(std::string_view name, const std::optional<std::function<void(const Pin&)>> onStateChange = std::nullopt, const TState& defaultState = TState())
    {
