...
Bus::Bus(const BusDriver& driver) noexcept
    : m_driver(driver)
    , m_syncRoot()
{
}
...
ScanResponse Bus::ExchangeInternal(const uint8_t address, const uint16_t data, const bool forceDataExchange) const noexcept
{
    m_driver.FlushInput();
