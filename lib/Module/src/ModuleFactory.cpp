...
std::unique_ptr<Module> ModuleFactory::CreateModule(Bus& bus, ModuleType type, uint8_t address, uint16_t initialData) noexcept
{
    ...
    if (type == ModuleType::Relais) return RelaisModule::CreateFromInitialData(bus, address, initialData);
